...
export const PaymentProvider = ({ children }: { children: ReactNode }) => {
  ...
  const payByPayhere = async (
    ...
    console.log(
      "fee, items, cus1, cus2, firstName, lastName, email, returnUrl, cancelUrl:",
      fee,
      items,
      cus1,
      cus2,
    ...
    try {
      ...
      const attachHandlersAndPay = () => {
        ...
        payhere.onCompleted = async function (orderId: string) {
          ...
          if (mode === "MENTORSHIP") {
            try {
              const { data } = await axiosMember.get(
                `/mentor/is-paid/${lastName}`
              );
              console.log("Mentorship payment status:", data);
            } catch (e) {
              console.warn("Mentorship status check failed:", e);
            }
          } // Handle Donation Payment Verification
            // We manually call the verify endpoint here to ensure the donation
            // is recorded in the database immediately after the user pays.
          else if (mode === "DONATION") {
            try {
              console.log("Verifying Donation for Order:", orderId);

              const { data } = await axiosGlobal.get(
                `/public/payment/notify/verify/${orderId}`
              );

              console.log("Donation saved:", data);
              toast.success("Donation recorded successfully!");
            } catch (e) {
              console.error("Donation verification failed:", e);
