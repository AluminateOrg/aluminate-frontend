...
export default function EventsPage() {
  ...
  const handleRSVP = async (eventId: string, status: 'yes' | 'no' | 'maybe') => {
    try {
      ...
      toast.success(`RSVP updated to "${status === 'yes' ? 'Going' : status === 'maybe' ? 'Maybe' : 'Not Going'}"`);
    } catch (error) {
  ...
  const pastEvents = events.filter(event => isBefore(new Date(event.startDate), new Date()));
